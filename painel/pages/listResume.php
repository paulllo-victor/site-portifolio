<?php
    $listResume = Site::resumes();
    //TODO FAZER A OPÇÃO PARA DELETAR
    if(isset($_GET['deletar'])){
        Resume::deleteResume($_GET['deletar']);
    }
?>
<div class="item-dashboard w100">
    ...
    <?php
        foreach ($listResume as $key => $value) {
            echo '
            <hr class="w100">
            <div class="left w33 item-list">'.$value['title'].'</div>
            <div class="left w33 item-list">'. $value['category'].'</div>
            <div class="left w33 item-list">
                <a class="btn update" href="'.PATH_PAINEL.'resume.php?editar='.$value['id'].'"><i class="fas fa-edit"></i> Editar</a>
